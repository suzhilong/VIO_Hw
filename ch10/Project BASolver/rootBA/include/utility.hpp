...
namespace rootBA {
    ...
    Matrix3<Scalar> SkewSymmetricMatrix(Vector3<Scalar> v) {
        Matrix3<Scalar> #pragma once
...
namespace rootBA {
    ...
    /* 计算四元数的左乘矩阵 */
    template<typename Scalar>
    Matrix4<Scalar> Qleft(Quaternion<Scalar> &q) {
        Matrix4<Scalar> Q;
    ...
    /* 计算四元数的右乘矩阵 */
    template<typename Scalar>
    Matrix4<Scalar> Qright(Quaternion<Scalar> &q) {
        Matrix4<Scalar> Q;
        Q.template block<1, 3>(0, 1) = - q.vec().transpose();
        Q.template block<3, 1>(1, 0) = q.vec();
        Q.template block<3, 3>(1, 1) = - SkewSymmetricMatrix(q.vec());
