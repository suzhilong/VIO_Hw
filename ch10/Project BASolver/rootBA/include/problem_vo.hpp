...
namespace rootBA {
    ...
    class ProblemVO {
        using LandmarkPtr = std::shared_ptr<LandmarkBlock<Scalar>>;
    ...
    public:
        ...
        bool AddLandmark(const std::shared_ptr<VertexLandmarkPosi<Scalar>> &newLandmark,
...
namespace rootBA {
    ...
    /* 必须在所有 camera 节点和 expose 节点都添加之后才可以添加 landmark */
    template<typename Scalar>
    bool ProblemVO<Scalar>::AddLandmark(const std::shared_ptr<VertexLandmarkPosi<Scalar>> &newLandmark,
        const std::unordered_map<size_t, std::shared_ptr<CameraObserve<Scalar>>> &observes) {
        ...
        LandmarkPtr landmarkBlock(new LandmarkBlock<Scalar>(newLandmark, observes, this->exPose));
...
namespace rootBA {
    ...
    bool ProblemVO<Scalar>::Solve(size_t maxIteration) {
        ...
        std::cout << "<Finish> Problem solve totally cost " << sumTimer.toc() << " ms\n";
        return true;
    }
    ...
    bool ProblemVO<Scalar>::LinearlizeLandmarkBlocks(void) {
        this->rowIndices.resize(this->landmarkBlocks.size());
        this->rowsof_matrix_J += tbb::parallel_reduce(tbb::blocked_range<size_t>(0, landmarkBlocks.size()), Scalar(0),
            [&] (tbb::blocked_range<size_t> range, Scalar temp) {
                for (size_t i = range.begin(); i < range.end(); ++i) {
                    // 清空 storage 矩阵
                    this->rowIndices[i] = this->landmarkBlocks[i]->ResizeStorage();
                    temp += this->rowIndices[i];
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(1, landmarkBlocks.size()),
            [&] (tbb::blocked_range<size_t> range) {
                for (size_t i = range.begin(); i < range.end(); ++i) {
                    this->rowIndices[i] -= this->rowIndices[0];
                }
            }
        );
        this->rowIndices[0] = 0;
        ...
        size_t rowIndex = tbb::parallel_reduce(tbb::blocked_range<size_t>(0, this->landmarkBlocks.size()), Scalar(0),
            [&] (tbb::blocked_range<size_t> range, size_t rows) {
                for (size_t i = range.begin(); i < range.end(); ++i) {
                    ...
                    auto scale = std::sqrt(this->landmarkBlocks[i]->GetWeight());
                    this->matrix_J.block(this->rowIndices[i], 0, res.Q2T_Jex.rows(), res.Q2T_Jex.cols()) = res.Q2T_Jex * scale;
                    this->vector_r.segment(this->rowIndices[i], res.Q2T_r.rows()) = res.Q2T_r * scale;
                    for (auto &item : res.Q2T_Jps) {
                        this->matrix_J.block(this->rowIndices[i], item.first * 6 + 6, item.second.rows(), item.second.cols()) = item.second * scale;
                    }
                    rows += res.Q2T_r.rows();
                }
                return rows;
    ...
    bool ProblemVO<Scalar>::SolveIncrementalFunction(void) {
        // 备份未添加阻尼因子的 H 矩阵的对角线元素，为 H #pragma once
...
namespace rootBA {
    ...
    bool ProblemVO<Scalar>::SolveIncrementalFunction(void) {
        ...
        this->delta_X.resize(this->matrix_H.cols() + this->landmarkBlocks.size() * 3, 1);
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(0, this->landmarkBlocks.size()),
            [&] (tbb::blocked_range<size_t> range) {
                for (size_t i = range.begin(); i < range.end(); ++i) {
                    this->delta_X.segment(this->matrix_H.rows() + i * 3, 3) =
    ...
    bool ProblemVO<Scalar>::IsValidStep(Scalar threshold) {
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(0, this->landmarkBlocks.size()),
            [&] (tbb::blocked_range<size_t> range) {
                for (size_t i = range.begin(); i < range.end(); ++i) {
                    b.segment(this->vector_b.rows() + 3 * i, 3) = - this->landmarkBlocks[i]->Get_Q1T_r();
                }
            }
        );
        ...
        if (this->policy == DampPolicy::Manual) {
            ...
            for (size_t i = 0; i < this->landmarkBlocks.size(); ++i) {
                diag.segment(this->vector_b.rows() + 3 * i, 3) = this->landmarkBlocks[i]->Get_R1().diagonal();
            }
            Scalar scale = this->delta_X.transpose() * (this->lambda * diag.asDiagonal() * this->delta_X + b);
            this->rho = (this->linearizedCost - this->sumCost) / (scale + 1e-6);
...
namespace rootBA {
    ...
    bool ProblemVO<Scalar>::CheckSize(const size_t priorSize) {
        ...
        // 但是行数本身算上了 damp 的固定 3 行，因此需要去掉
        this->rowsof_matrix_J -= 3 * this->landmarkBlocks.size();
        this->rowsof_matrix_J += this->prior_J.rows();
        ...
        size_t rowIndex = tbb::parallel_reduce(tbb::blocked_range<size_t>(0, this->landmarkBlocks.size()), Scalar(0),
            [&] (tbb::blocked_range<size_t> range, size_t rows) {
                for (size_t i = range.begin(); i < range.end(); ++i) {
                    ...
                    auto scale = std::sqrt(this->landmarkBlocks[i]->GetWeight());
                    this->matrix_J.block(this->rowIndices[i] - 3 * i, 0, res.Q2T_Jex.rows(), res.Q2T_Jex.cols()) = res.Q2T_Jex * scale;
                    this->vector_r.segment(this->rowIndices[i] - 3 * i, res.Q2T_r.rows()) = res.Q2T_r * scale;
                    for (auto &item : res.Q2T_Jps) {
                        this->matrix_J.block(this->rowIndices[i] - 3 * i, item.first * 6 + 6, item.second.rows(), item.second.cols()) = item.second * scale;
                    }
                    rows += res.Q2T_r.rows();
                }
                return rows;
